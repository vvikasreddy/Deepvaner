...
class FaceFeatureExtractor(nn.Module):
    def __init__(self, feature_size=16, pretrain=True):
        ...
        self.fc = nn.Linear(128, feature_size)
    ...
    def forward(self, x):
        ...
        # print(torch.any(torch.isnan(cnn_output)), "face, no 1")
        ...
        # print(torch.any(torch.isnan(rnn_output)), "face, no 2")
        rnn_output = torch.squeeze(rnn_output)

        output = self.fc(rnn_output)
        # print(torch.any(torch.isnan(output)), "face no 3")
        return output
...
class Transformer1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(torch.any(torch.isnan(x)), "number 3")
        return x
...
class DeepVANet(nn.Module):
    ...
    def forward(self,x):
        ...
        # print(torch.any(torch.isnan(img_features)), "img features")
        # print(torch.any(torch.isnan(bio_features)), "bio_features")

        img_features = img_features.unsqueeze(0)
        # print(img_features.size())
        # bio_features = bio_features.squeeze()
        # print(bio_features.size())
        features = torch.cat([img_features,bio_features],dim=1)
